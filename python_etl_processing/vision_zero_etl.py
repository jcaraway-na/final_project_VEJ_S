...
class api_calls():
    ...
    async def get_v0_data():
        ...
        # v0 uri end-point
        results = await client.get("y2wy-tgr5", limit=200000)
    ...
    async def get_crash_by_id(id):
        r = await request.get(url = f'{base_url}{get_by_id_uri}/{id}')
        
        return r

    async def post_request(payload):
        print(payload)
        r = await request.post(url = f'{base_url}{post_uri}', json=payload)
        
        return r

    async def put_request(id, payload):
        r = await vvrequest.post(url = f'{base_url}{put_uri}/{id}', json=payload)

        return r

    async def get_post_put_request(dataframe):
        for index, row in dataframe.iterrows():
            ...
            response = await api_calls.get_crash_by_id(crash_id)

            # if crash id != exist then post payload. else put payload
            if response.status_code == 204:
                print(f'Record id {crash_id} not found!')
                print(f'POST status code: {await api_calls.post_request(payload).status_code}.')
                ...
                response = await api_calls.put_request(crash_id,payload)
