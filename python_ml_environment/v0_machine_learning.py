...
#region total serious accidents api calls
# api base url
...
# get request end-point
get_all_totalseriousaccidents_uri = "TotalSeriousAccidents/get-all-totalseriousaccidentsperyear-data"

# get request by id end-point
get_totalseriousaccidents_by_id_uri = "TotalSeriousAccidents/get-totalseriousaccidentsperyear-by-id"

# post request end-point
post_totalseriousaccidents_uri = "TotalSeriousAccidents/add-totalseriousaccidentsperyear"

# put request by id end-point
put_totalseriousaccidents_uri = "TotalSeriousAccidents/update-totalseriousaccidentsperyear-by-id"

#endregion

#region get crash api calls
# get request end-point
get_all_crash_uri = "Crash/get-all-crash_data"

# get request by id end-point
get_crash_by_id_uri = "Crash/get-crash-by-id"

# post request end-point
post_crash_uri = "Crash/add-crash"

# put request by id end-point
put_crash_uri = "Crash/update-crash-by-id"
#endregion

#region prediction SRS api calls

# get request end-point
get_all_predictions = "PredictSrs/get-all-predictsrs-data"

# get request by id end-point
get_prediction_by_id_uri = "PredictSrs/get-predictsrs-by-id"

# post request end-point
post_predictions_uri = "PredictSrs/add-predictsrs"

# put request by id end-point
put_predictions_uri = "PredictSrs/update-predictsrs-by-id"
...
def get_all_request(uri):
    r = request.get(url = f'{base_url}{uri}')
    
    response = r.json()
    
    return response

def get_by_id_request(uri,id):
    r = request.get(url = f'{base_url}{uri}/{id}')
    
    return r

def post_request(uri,payload):
    r = request.post(url = f'{base_url}{uri}', json=payload)
    
    return r

def put_request(uri,id,payload):
    r = request.put(url = f'{base_url}{uri}/{id}', json=payload)
...
v0_df = pd.DataFrame(get_all_request(get_all_crash_uri))
...
pop_df = pd.DataFrame(get_all_request(get_pop_uri))
print(pop_df)
...
def predict_srs(start_year, end_year, start_month, end_month):
    ...
    print(results_df)
    results_df.to_csv(f'./ml_csv/Serious_{start_year}.csv')

    # # upload Predict Srs data
    # payload = {}
    # for index,row in results_df.iterrows():

    #     # build out payload
    #     payload = {'Month': str(row['Month']),'Prediction': int(row['Prediction']),
    #     'Actual': int(row['Actual']), 'Crashes_Avoided': int(row['Crashes_Avoided'])}

    #     print(f'POST status code: {post_request(post_predictions_uri,payload).status_code}.')
...
predict_srs(2020, 2021, 0, 13)
predict_srs(2021, 2022, 0, 13)
...
def predict_pedestrians(start_year, end_year, start_month, end_month):
    ...
    results_df.to_csv(f'./ml_csv/Pedestrians_{start_year}.csv')
...
# Look at April-Sep 2020
predict_pedestrians(2020, 2021, 0,13)
predict_pedestrians(2021, 2022, 0, 13)
...
hours_comparison.to_csv(f'./ml_csv/Crash_by_Hour.csv')
...
compare_dayofweek_df.to_csv(f'./ml_csv/Crash_Day_of_Week.csv')
...
months_comparison.to_csv(f'./ml_csv/Crash_by_Month.csv')
...
pop_grow = people_df[['year_month', 'population', 'growth_rate']]
...
totals_pop.to_csv(f'./ml_csv/Control_for_Pop.csv')